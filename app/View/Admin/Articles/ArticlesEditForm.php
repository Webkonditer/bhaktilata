...
class ArticlesEditForm
{
    ...
    public function fill(Article $newsItem)
    {
        ...
        $this->form()->setAction(route('admin.articles.store', ['newsItem' => $newsItem]));
    }
    ...
    private function createForm(): Form
    {
        ...
        $form->addField(new FormField\StringFormField('author', 'Автор', [
            'rules' => ['string|max:191|required'],
        ]));
        $form->addField(new FormField\DateFormField('date', 'Дата публикации', [
            'rules' => ['date|date_format:d.m.Y|required']
        ]));
        $form->addField(new FormField\StringFormField('announce', 'Описание', [
            'rules' => ['max:191'],
        ]));
